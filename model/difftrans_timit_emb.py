...
class DIFFUSION_BIAS(nn.Module):
    ...
    def __init__(self,feature_dim,id_dim):
        ...
        self.z_size=self.denoiser.latent_dim*self.denoiser.transformer_decoder.num_layers
        self.speaker_embedding = nn.Sequential(
            nn.Embedding(id_dim, self.z_size), #latent_dim * num_layers)
    ...
    def _diffusion_reverse(
        ...
        if id is not None:
            object_emb = self.speaker_embedding(torch.argmax(id, dim = 1)).unsqueeze(1)
            emb_mu=self.speaker_mu(object_emb)
            emb_logvar=self.speaker_logvar(object_emb)
            std=torch.exp(0.5*emb_logvar)
            eps=torch.randn_like(std)
            object_emb=emb_mu+eps*std
        else:
            object_emb=None
        # sample noise
