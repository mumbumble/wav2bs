...
if __name__ == "__main__":
    ...
    model_path = "./output/model/save_difftrans_large"
    template_path = 'template.json'

    curve_norm = np.load(os.path.join(".", 'curve_norm_etc.npy'),allow_pickle=True).tolist()

    #train
    model=DIFFUSION_BIAS(feature_dim,id_dim)
    model = model.to(torch.device("cuda"))
    model.load_state_dict(torch.load(os.path.join(model_path,'1300_model.pth')))
    model.eval()
    ...
    for wav_filename in tqdm(wav_filelist):
        ...
        for id,speaker_id_name in [
            ...
            with torch.no_grad():
                predictdata = model.predict(torch.tensor(wav).to(device="cuda"), iteration, fps,id= id)
            ...
            json.dump(output, open(os.path.join('./out/wav_large',output_name+"_"+speaker_id_name+".json"),'wt'))
